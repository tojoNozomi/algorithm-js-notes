...
动态规划也就是一种将复杂问题分解成更小的子问题来解决的优先技术，但是和分而治之相比还是有些区别的。分治是将问题分解成相互独立的子问题，然后组合它们的解；而DP则是将问题分解成相互依赖的问题。（直观上就是并行和串行的区别？这里需要补个图）
...
使用DP解决问题的话，需要在每一个阶段（子问题）中做出决策，各个阶段的决策的选取只依赖于当前状态（这里的当前状态指的是当前的输入状态），从而确定输出状态。当每个阶段的决策确定之后，形成了一个决策序列，也就决定了问题的最终解决方案。这种将问题看作一个前后关联具有链状结构的多阶段过程称为多阶段决策问题。



DP问题会常涉及到状态转移方程的概念。如果要用前一个阶段的结果来解决后一个阶段的问题，就需要建立起前后阶段状态的转移关系，最好是能用一系列方程来表示。



在衡量问题的最优解的时候需要一个指标函数。最优解就是让这个函数达到最优值，如最大或者最小。如果可以将问题拆分成子问题，那么这个指标函数就必须要具有分离性。也就是要能够利用子问题的最优解递推出问题的最优解。当整体求出最优解也就可以知道各个子问题的最优解。



上面提及的“近依赖于当前状态”，指的是问题的历史状态不会影响其未来发展，只能通过当前状态去影响未来，当前状态就是对历史的一个总结。这个性质也称为无后效性（马尔科夫性）。比如阶段2的决策只由阶段2的输入状态决定，而与阶段1的输入状态无关。



解题步骤：

1.  将问题拆分成多阶段子问题，确定好每个阶段的输入和输出。
2. 确定决策集合，比如当前子问题里，会有怎么样的输出。
3. 确定状态转移方程，分离指标函数。
...
定义子问题的时候，这些相同类型的子问题就称为重叠子结构；在算法中，按照自底向上的策略，首先找到子问题的最优解，	解决子问题，然后逐步向上找到问题的一个最优解，也就是整体问题的最优解已经包含了子问题的最优解，即称为最优子结构。
